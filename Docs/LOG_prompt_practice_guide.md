...
## 5. UI/UX 개선 (2026-05-11)
- **대화 및 슬라이드 연동 로직 개선:** 대화 내용이 출력 중이거나 마지막 대화에 도달하기 전에는 하단의 '다음 슬라이드' 버튼(`btn-next`)을 비활성화(`opacity: 0.3`, `pointer-events: none`)하여 사용자가 대화를 끝까지 읽도록 유도함.
- **안내 메시지 추가:** 마지막 대화 출력이 끝나면 대화창 내에 `(대화가 모두 끝났습니다. 하단의 다음 슬라이드 버튼을 클릭하세요.)`라는 시스템 메시지가 표시되도록 수정. 동시에 '다음 슬라이드' 버튼이 활성화됨.
- **중앙 포커스 카드 추가 (RCTF 슬라이드):** Stage 1(RCTF) 진행 중 R, C, T, F 각 항목에 대해 대화가 나올 때 화면 중앙(좌측 중앙)에 글래스모피즘 스타일의 포커스 카드(`#center-focus-card`)가 나타나 사용자가 핵심 키워드를 직관적으로 인지할 수 있도록 시각적 요소를 강화함.
